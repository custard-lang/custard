...
- 再帰呼び出しの禁止: 発散コンビネーターを禁止できていないっぽいので対策を考える
    - 恐らく、簡易的な型チェックをしないとできなさそう
...
- [ ] テスト: 関数名とその引数の名前が被っているケース
    (varName)
- [ ] `Context`を受け取るマクロ: 特殊な、contextを使うことを示すモジュールを、直接あるいは間接に`import`している関数の場合は呼び出し時に第1引数として`Context`を渡すようにすればいいんだ。将来作る予定のモジュールシステムの仕様とも整合する
- [ ] 入れ子のsplice処理をした場合の仕様
