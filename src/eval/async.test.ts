...
describe("evalForm", () => {
  ...
  describe("async.await", () => {
    testEvalFormOf({
      ...
      expected: new TranspileError("Top-level `await` is not supported yet. Sorry."),
      setUpReplOptions,
      preludeSrc,
    });
    testEvalFormOf({
      src: '(scope (async.await (Promise.resolve 5)))',
      expected: new TranspileError("`await` in a non-async function or scope is not allowed."),
      setUpReplOptions,
      preludeSrc,
    });
  });
