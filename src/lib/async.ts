...
import { Env, Form, markAsDirectWriter, TranspileError } from "../internal/types.js";
import { JsSrc } from "../types.js";
import { buildFn, buildProcedure, buildScope, transpiling1Unmarked } from "./base/common.js";

export const _cu$await = markAsDirectWriter(
  (env: Env, a: Form, ...unused: Form[]) => {
    if (EnvF.isAtTopLevel(env)) {
      return new TranspileError("Top-level `await` is not supported yet. Sorry.");
    }
    if (!EnvF.isInAsyncContext(env)) {
      return new TranspileError("`await` in a non-async function or scope is not allowed.");
    }
    return transpiling1Unmarked("await", (s: JsSrc) => `await ${s}`)(env, a, ...unused);
  }
);
